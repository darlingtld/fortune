...
import fortune.pojo.ZoufeiStub;
...
public class UserDao {
    ...
    public void switchZoufeiStatus(String groupId, String userId) {
        User user = getUserById(userId);
        ZoufeiStub zoufeiStub = user.getZoufeiEnabledMap().get(groupId);
        if (zoufeiStub == null) {
            zoufeiStub = new ZoufeiStub();
            zoufeiStub.isZoufeiAutoEnabled = true;
            zoufeiStub.thresholdStakes = 0;
        }
        zoufeiStub.isZoufeiAutoEnabled = !zoufeiStub.isZoufeiAutoEnabled;
        user.getZoufeiEnabledMap().put(groupId, zoufeiStub);
        Query query = new Query(Criteria.where("id").is(user.getId()));
        Update update = new Update();
    ...
    public void saveZoufeiSetting(String groupId, String userId, ZoufeiSetting zoufeiSetting) {
        User user = getUserById(userId);
        ZoufeiStub zoufeiStub = user.getZoufeiEnabledMap().get(groupId);
        if (zoufeiStub == null) {
            zoufeiStub = new ZoufeiStub();
            zoufeiStub.isZoufeiAutoEnabled = true;
            zoufeiStub.thresholdStakes = zoufeiSetting.getCurStake();
        }
        zoufeiStub.thresholdStakes = zoufeiSetting.getCurStake();
        user.getZoufeiEnabledMap().put(groupId, zoufeiStub);
        Query query = new Query(Criteria.where("id").is(user.getId()));
